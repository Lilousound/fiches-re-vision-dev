...
### **📌 Exercice 1 : Créer une API REST Simple avec Node.js + Express**
**Consigne** : Crée une API REST pour gérer une liste de tâches (To-Do List) avec les fonctionnalités suivantes :
1. **Lister toutes les tâches** (`GET /taches`).
2. **Créer une tâche** (`POST /taches`).
3. **Récupérer une tâche par ID** (`GET /taches/:id`).
4. **Mettre à jour une tâche** (`PUT /taches/:id`).
5. **Supprimer une tâche** (`DELETE /taches/:id`).
...
// Données en mémoire (à remplacer par une base de données en production)
let taches = [
  { id: 1, titre: 'Apprendre REST', terminee: false },
  { id: 2, titre: 'Créer une API', terminee: false },
];
...
app.post('/taches', (req, res) => {
  const { titre, terminee = false } = req.body;
  if (!titre) {
    return res.status(400).json({ error: 'Le titre est obligatoire.' });
  }
  const nouvelleTache = {
    id: taches.length > 0 ? Math.max(...taches.map(t => t.id)) + 1 : 1,
    titre,
    terminee,
  };
...
app.put('/taches/:id', (req, res) => {
  ...
  taches[index] = { ...taches[index], titre, terminee };
...
3. Tester avec **Postman** ou **cURL** :
  ```bash
   ...
   # Créer une tâche
   curl -X POST -H "Content-Type: application/json" -d '{"titre": "Tester l\'API"}' http://localhost:3000/taches

   # Récupérer une tâche
   curl http://localhost:3000/taches/1

   # Mettre à jour une tâche
   curl -X PUT -H "Content-Type: application/json" -d '{"titre": "Tester l\'API (mis à jour)", "terminee": true}' http://localhost:3000/taches/1

   # Supprimer une tâche
   curl -X DELETE http://localhost:3000/taches/1
  ```

---

### **📌 Exercice 2 : Documenter une API avec Swagger/OpenAPI**

**Consigne** : Documenter l’API de l’exercice 1 avec **Swagger/OpenAPI** (format YAML).
...
info:
  ...
  description: API REST pour gérer une liste de tâches (To-Do List).
  version: 1.0.0
servers:
          ...
          application/json:
            schema:
              ...
              properties:
                titre:
                  type: string
                  example: "Apprendre REST (mis à jour)"
                terminee:
                  type: boolean
                  example: true
              required:
...
1. Copier ce YAML dans un fichier `openapi.yaml`.
2. Utiliser [Swagger Editor](https://editor.swagger.io/) pour visualiser et tester l’API.

---

### **📌 Exercice 3 : Appeler une API Externe avec JavaScript (Fetch)**

**Consigne** : Utiliser l’API publique [JSONPlaceholder](https://jsonplaceholder.typicode.com/) pour :

1. Récupérer la liste des utilisateurs (`GET /users`).
2. Récupérer les posts d’un utilisateur spécifique (`GET /users/{id}/posts`).
...
// 2. Récupérer les posts de l'utilisateur avec l'ID 1
...
async function fetchUsersAndPosts() {
  try {
    ...
    // 2. Récupérer les posts de l'utilisateur 1
...
### **📌 Exercice 4 : Créer un Client GraphQL avec Apollo (React)**

**Consigne** : Utiliser l’API GraphQL publique [SpaceX](https://api.spacex.land/graphql/) pour :

1. Récupérer la liste des missions (`launches`).
2. Afficher les 5 premières missions dans une application React.
...
2. **Configurer Apollo Client** :
  ```tsx
   ...
   export default client;
  ```
3. **Créer un composant pour afficher les missions** :
  ```tsx
   ...
   export default function Missions() {
     ...
     return (
       <div>
         <h1>Prochaines Missions SpaceX</h1>
         <ul>
  ...
  ```
4. **Intégrer Apollo dans ton application** :
  ```tsx
   // src/App.tsx
   import { ApolloProvider } from '@apollo/client';
   import client from './apolloClient';
