...
## **🔹 16. Exercice Pratique (TypeScript)**

**Consigne** : Crée un **système de gestion de tâches (To-Do List)** en TypeScript avec :

1. Une **interface `Tache**` avec `id`, `texte`, `terminee`, et `dateCreation`.
2. Une **classe `GestionTaches**` avec :
  - Une propriété `taches` (tableau de `Tache`).
  - Une méthode `ajouterTache` pour ajouter une tâche.
  - Une méthode `supprimerTache` pour supprimer une tâche par `id`.
  - Une méthode `basculerTache` pour basculer l’état `terminee` d’une tâche.
  - Une méthode `filtrerTaches` pour filtrer les tâches (toutes, actives, terminées).
3. Une **fonction générique `premierElement**` qui retourne le premier élément d’un tableau (de n’importe quel type).
4. Une **fonction `afficherTaches**` qui affiche les tâches dans la console.
5. **Utilise des types utilitaires** (`Pick`, `Omit`, `Partial`) pour créer des types dérivés de `Tache`.
...
// 2. Types dérivés avec les types utilitaires
type TachePartielle = Partial<Tache>;
type TacheSansId = Omit<Tache, "id">;
type TacheIdEtTexte = Pick<Tache, "id" | "texte">;

// 3. Classe GestionTaches
class GestionTaches {
  private taches: Tache[];

  constructor(taches: Tache[] = []) {
    this.taches = taches;
  }

  ajouterTache(texte: string): Tache {
  ...
  supprimerTache(id: number): boolean {
  ...
  basculerTache(id: number): boolean {
  ...
  filtrerTaches(filtre: "toutes" | "actives" | "terminees"): Tache[] {
    switch (filtre) {
      case "actives":
...
// 4. Fonction générique premierElement
function premierElement<T>(tableau: T[]): T | undefined {
  return tableau[0];
}

// 5. Fonction afficherTaches
function afficherTaches(taches: Tache[]): void {
  ...
  taches.forEach(tache => {
    ...
    console.log(`${statut} ${tache.texte} (ID: ${tache.id}, Créée le: ${tache.dateCreation.toLocaleDateString()})`);
  });
}

// Tests
const gestion = new GestionTaches();

gestion.ajouterTache("Apprendre TypeScript");
gestion.ajouterTache("Créer un projet");
gestion.ajouterTache("Faire un exercice");

console.log("Toutes les tâches :");
afficherTaches(gestion.getTaches());

gestion.basculerTache(gestion.getTaches()[0].id);
console.log("\nTâches actives :");
afficherTaches(gestion.filtrerTaches("actives"));

gestion.supprimerTache(gestion.getTaches()[1].id);
console.log("\nTâches après suppression :");
afficherTaches(gestion.getTaches());

// Test de la fonction générique
const premier = premierElement(gestion.getTaches());
console.log("\nPremière tâche :", premier?.texte);

// Test des types dérivés
const tachePartielle: TachePartielle = { texte: "Tâche partielle" };
const tacheSansId: TacheSansId = { texte: "Tâche sans ID", terminee: false, dateCreation: new Date() };
const tacheIdEtTexte: TacheIdEtTexte = { id: 1, texte: "Tâche avec ID et texte" };
```
