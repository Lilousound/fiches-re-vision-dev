...
## **🔹 17. Exercice Pratique (React)**

**Consigne** : Crée une **application de gestion de tâches (To-Do List)** avec :

1. Un **champ de saisie** pour ajouter une nouvelle tâche.
2. Un **bouton** pour ajouter la tâche.
3. Une **liste** de tâches avec :
  - Un **bouton "Supprimer"** pour chaque tâche.
  - Un **bouton "Terminer"** pour marquer une tâche comme terminée (barre le texte).
  - Un **filtre** pour afficher toutes les tâches, seulement les actives, ou seulement les terminées.
4. Un **compteur** qui affiche le nombre de tâches restantes.
5. **Stockage local** : Utilise `localStorage` pour sauvegarder les tâches entre les sessions.
6. **Utilise des hooks personnalisés** pour gérer la logique des tâches et du stockage.
...
export function useTaches() {
  const [taches, setTaches] = useState(() => {
    ...
    return savedTaches ? JSON.parse(savedTaches) : [];
  });
  ...
  const tachesActives = taches.filter(tache => !tache.terminee).length;
...
```jsx
// src/components/Tache.jsx
import React from 'react';

export default function Tache({ tache, onSupprimer, onBasculer }) {
  return (
    <li className={tache.terminee ? 'terminee' : ''}>
      <span>{tache.texte}</span>
      <div className="actions">
        <button onClick={() => onBasculer(tache.id)}>
          {tache.terminee ? 'Réactiver' : 'Terminer'}
        </button>
...
```jsx
// src/components/FiltreTaches.jsx
import React from 'react';

export default function FiltreTaches({ filtre, onFiltreChange }) {
  return (
    <div className="filtre">
      <button
        onClick={() => onFiltreChange('toutes')}
        className={filtre === 'toutes' ? 'actif' : ''}
      >
        Toutes
      </button>
      <button
        onClick={() => onFiltreChange('actives')}
        className={filtre === 'actives' ? 'actif' : ''}
      >
        Actives
      </button>
      <button
        onClick={() => onFiltreChange('terminees')}
...
import FiltreTaches from './components/FiltreTaches';
import './App.css';
...
function App() {
  ...
  const tachesFiltrees = filtrerTaches(filtre);
  ...
  return (
    <div className="app">
      ...
      <form onSubmit={handleSubmit} className="ajout-tache">
        <input
          type="text"
          value={texteTache}
      ...
      <FiltreTaches filtre={filtre} onFiltreChange={setFiltre} />

      <p className="compteur">
        {tachesActives} tâche{tachesActives !== 1 ? 's' : ''} restante{tachesActives !== 1 ? 's' : ''}
      </p>
...
.liste-taches li .actions {
  display: flex;
  gap: 5px;
}

.liste-taches li .actions button {
  padding: 5px 10px;
  font-size: 12px;
  background: #f0f0f0;
  border: 1px solid #ddd;
...
.liste-taches li .actions button:hover {
  background: #e0e0e0;
}
```
