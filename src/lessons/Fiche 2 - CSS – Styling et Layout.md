...
## **🔹 13. Exercice Pratique (CSS)**

**Consigne** : Crée une page CSS complète avec :

1. Un **en-tête** (`<header>`) avec :
  - Un titre centré.
  - Un menu de navigation horizontal (Flexbox) avec 3 liens.
  - Un bouton "Contact" aligné à droite.
2. Un **contenu principal** (`<main>`) avec :
  - Une **bannière** (`<section>`) avec une image de fond (`background-image`) et un texte centré verticalement et horizontalement (Flexbox ou Grid).
  - Une **grille de cartes** (`<section>`) avec 3 cartes (CSS Grid) :
    - Chaque carte a une image, un titre, et un texte.
    - Les cartes doivent être **responsive** (s’adapter à la taille de l’écran).
3. Un **pied de page** (`<footer>`) avec :
  - Un formulaire de newsletter (champ email + bouton).
  - Des liens vers les réseaux sociaux (icônes alignées horizontalement).
4. **Effets** :
  - Les liens du menu changent de couleur au survol (`:hover`).
  - Les cartes ont une **ombre** (`box-shadow`) et un **effet de survol** (ex : `transform: scale(1.05)`).
  - Le bouton du formulaire a une **transition** (couleur de fond).
